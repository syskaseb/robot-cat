"""Read-only world-shape extraction from the saved, resting v31 checkpoint."""
...
for row in plan['components']:
    ...
    assert s.isValid() and s.Solids,row['name'];s.exportBrep(str(cache/(o.Name+'.brep')))
...
print('Exported',len(plan['components']),'world BReps; source untouched')

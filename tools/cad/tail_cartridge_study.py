...
def revise_fasteners():
    """Derive nominal M2x10 envelope from existing M2x12; no supplier claim."""
    import FreeCAD as A
    import Part
    assert A.GuiUp
    d=A.getDocument('TailHornCartridge');p=A.getDocument('TailPurchased')
    for i,(x,y) in enumerate(HOLES):
        b=pilot.global_shape(p.getObject('BoltM2x12'))
        b=b.cut(Part.makeBox(10,10,5,A.Vector(-5,-5,10)))
        b.translate(A.Vector(x,y,90.2))
        d.getObject(f'CartridgeBolt{i}').Shape=b
        d.getObject(f'CartridgeBolt{i}').Provenance='Nominal M2x10 envelope derived by shortening copied M2x12 shaft by 2 mm; NOT a measured supplier model'
        n=pilot.global_shape(p.getObject('NutM2'));n.translate(A.Vector(x,y,98))
        d.getObject(f'CartridgeNut{i}').Shape=n
    for o in d.Objects:
        if hasattr(o,'ViewObject'):o.ViewObject.Visibility=False
    for n in PRINTS+['CommunityHornEnvelope']+[f'Cartridge{kind}{i}' for kind in ('Bolt','Nut') for i in range(4)]:
        d.getObject(n).ViewObject.Visibility=True
    d.recompute();d.Label='Tail horn cartridge R1 - CONCEPT NOT INSTALLED';d.save()
...
def audit():
    ...
    try:
        ...
        report['fastener_stack_mm']={'head_seat_z':90.2,'lower_boss_above_seat':2,'upper_carrier_above_boss':1.8,'root_seat':4,'nominal_nut':1.6,'bolt_length':10,'protrusion':.6}
...
if __name__ == "__main__":
    {"survey": survey, "populate":populate, "revise_fasteners":revise_fasteners,"audit":audit}[sys.argv[1]]()

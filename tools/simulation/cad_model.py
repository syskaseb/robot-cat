...
assert REVISION in ('v25','v27','v28','v29','v30','v31'), REVISION
...
def make_model(geometry,**mass_options):
    ...
    return dict(source_sha256=geometry['source_sha256'], components=rows, links=links, joints=axes,
                total=combine(rows), warnings=[
                    (REVISION+f" snapshot: head/tail fixed in physics; three auxiliary servos, one supplier tail STEP and two head placeholders; {geometry['temporary_locks']} native temporary locks." if REVISION in ('v29','v30','v31') else
                     REVISION+' snapshot: head/tail fixed, 4 auxiliary placeholders, 64 temporary locks.'),
                    'MG92B total nominal 13.8 g; 12.8 g case / 1 g output split is unmeasured.' if REVISION in ('v29','v30','v31') else 'Legacy auxiliary envelopes are not actual mount validation.',

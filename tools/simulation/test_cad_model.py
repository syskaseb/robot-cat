...
def test_v29_auxiliary_and_fastener_mass_classification(geometry):
    if REVISION not in ('v29','v30','v31','v32','v33','v34','v35'):pytest.skip('v29+ tail integration only')
...
def test_v30_imu_parts_are_rigidly_accounted_for(geometry):
    if REVISION not in ('v30','v31','v32','v33','v34','v35'):pytest.skip('v30+ IMU mounting only')
    rows={r['name']:r for r in mass_budget(geometry)}
    assert len(geometry['components'])=={'v30':264,'v31':280,'v32':286,'v33':289,'v34':306,'v35':320}[REVISION]
...
def test_v31_power_mount_mass_classification(geometry):
    if REVISION not in ('v31','v32','v33','v34','v35'):pytest.skip('v31+ main regulator mounting only')
    rows={r['name']:r for r in mass_budget(geometry)}
...
def test_v32_tof_hardware_and_head_budget(geometry):
    if REVISION not in ('v32','v33','v34','v35'):pytest.skip('v32+ ToF mounting only')
    rows={r['name']:r for r in mass_budget(geometry)}
...
def test_v33_integral_face_and_purchased_microphone_accounting(geometry):
    if REVISION not in ('v33','v34','v35'):pytest.skip('v33+ integral face and microphone mount only')
    rows={r['name']:r for r in mass_budget(geometry)}
...
def test_v33_microphone_holes_follow_actual_supplier_pose():
    if REVISION not in ('v33','v34','v35'):pytest.skip('v33+ mounting plan only')
...
def test_v33_head_mesh_proof_is_current_and_not_a_release():
    if REVISION not in ('v33','v34','v35'):pytest.skip('v33+ inherited head recovery only')
    import hashlib
